...
class sstcalcualtion(models.Model):
    ...
    def get_from_date_digit(self,i,f):
        ...
        _logger.info("Date without convert=================%s"%self.from_date)
        _logger.info("DATEEEEE88***************************************%s"%date_time_from)
        if date_time_from:
            if i==0 and f==f:
            ...
            if i==4 and f==f:
                return date_time_from[6]
            if i==5 and f==f:
                return date_time_from[7]

    # def get_from_date_digit(self, i, f):
    #     """Return a specific digit from from_date in DDMMYYYY format.
    #     Indices: 0,1=Day | 2,3=Month | 4,5=Year(last 2 digits)
    #     """
    #     if not self.from_date:
    #         return ''
    #
    #     date_str = self.from_date.strftime("%d%m%Y")
    #     mapping = {0: 0, 1: 1, 2: 2, 3: 3, 4: 6, 5: 7}
    #
    #     if i in mapping:
    #         return date_str[mapping[i]]
    #     return ''
    ...
    def get_to_date_digit(self, i, t):
        ...
        if date_time_to:
            ...
            if i == 4 and t == t:
                return date_time_to[6]
            if i == 5 and t == t:
                return date_time_to[7]
    ...
    def get_due_date_digit(self,i,d):
        ...
        if date_time_due:
            ...
            if i == 4 and d == d:
                return date_time_due[6]
            if i == 5 and d == d:
                return date_time_due[7]
    ...
    def calculate_sst_service_lines(self):
        ...
        hs_code_obj=self.env['product.hs_code'].search([])
        number=1
        if self.state=='process':
            ...
            for hs_code in hs_code_obj:
                account_move_line_obj=self.env['account.move.line'].search([('move_id.invoice_date','>=',self.from_date),('move_id.invoice_date','<=',self.to_date),
                        ('move_id.state','=','posted'),('product_id.hs_code_id','=',hs_code.id),('move_id.move_type','in',['out_invoice'])])
                ...
                for line in account_move_line_obj:
                    if line.tax_ids:
                        ...
                        if line.tax_ids.tax_code_id.name in ['SU-10','SU-5']:
                            # own_sold_amt+=(line.quantity*line.price_unit)
                            own_sold_amt+=abs(line.debit - line.credit)
                        if line.tax_ids.tax_code_id.name in ['S-10','S-5']:
                            # good_sold_amt+=(line.quantity*line.price_unit)
                            good_sold_amt+=abs(line.debit - line.credit)
                    else:
                        pass

                print('good_sold_amt',good_sold_amt)
                print('own_sold_amt',own_sold_amt)
                # service_vals={'tariff_code':hs_code.name,'subtotal':(good_sold_amt,2),'description':hs_code.description,'number':number,
                #               'sst_id':self.id or self.active_id.id,'free_service':round(own_sold_amt,2)}
                service_vals={'tariff_code':hs_code.name,'subtotal':(good_sold_amt),'description':hs_code.description,'number':number,
                              'sst_id':self.id or self.active_id.id,'free_service':(own_sold_amt)}                              
                print('len',len(self.sst_service_ids),len(self.sst_payable_ids))
                c=self.sst_service_ids.create(service_vals)
                number+=1

            #calculate amounts for without hscode product invoices
            account_move_line_obj_without_hscode = self.env['account.move.line'].search(
                [('move_id.invoice_date', '>=', self.from_date), ('move_id.invoice_date', '<=', self.to_date),
                 ('move_id.state', '=', 'posted'),
                 ('move_id.move_type','in',['out_invoice'])])
            good_sold_amt1 = 0
            own_sold_amt1 = 0
            for line in account_move_line_obj_without_hscode:
                print('line_hscode',line)
                if not line.product_id.hs_code and line.tax_ids:
                    _logger.info("=====================invoicess======%s" % line.move_id.name)
                    if line.tax_ids.tax_code_id.name in ['SU-10', 'SU-5']:
                        # own_sold_amt+=(line.quantity*line.price_unit)
                        own_sold_amt1 += abs(line.debit - line.credit)
                    if line.tax_ids.tax_code_id.name in ['S-10', 'S-5']:
                        # good_sold_amt+=(line.quantity*line.price_unit)
                        good_sold_amt1 += abs(line.debit - line.credit)
                else:
                    pass

            service_vals = { 'subtotal': (good_sold_amt1),
                            'description': 'OTHERS', 'number': number,
                            'sst_id': self.id or self.active_id.id, 'free_service': (own_sold_amt1)}                            
            print('len', len(self.sst_service_ids), len(self.sst_payable_ids))
            c = self.sst_service_ids.create(service_vals)
            ...
            # tax_payable_amt1=round((value_taxable_sales_amt_ten * 0.10),3)
            # _logger.info('tax_payable_amt1=%s'%tax_payable_amt1)
            # payable_list.append(
            #     [0, 0, {'label': 'Taxable Good at 10% Rate', 'value_taxable_sales_amt': value_taxable_sales_amt_ten,
            #             'tax_rate': '10%', 'tax_payable_amt': tax_payable_amt1}])

            for line in account_move_line_obj_payable:
                if line.tax_line_id:
                    # for tax in line.tax_ids:
                    if line.tax_line_id.tax_code_id.name in ['SU-5', 'S-5']:
                        tax_five_percentage_amt += abs(line.debit-line.credit)
                    elif line.tax_line_id.tax_code_id.name in ['SU-10','S-10']:
                        tax_ten_percetnage_amt += abs(line.debit-line.credit)
                else:pass
                if line.tax_ids:
                    if line.tax_ids.tax_code_id.name in ['SU-10', 'SU-5']:
                        # own_sold_amt+=(line.quantity*line.price_unit)
                        value_taxable_sales_amt_five_new += abs(line.debit - line.credit)
                    if line.tax_ids.tax_code_id.name in ['S-10', 'S-5']:
                        # good_sold_amt+=(line.quantity*line.price_unit)
                        value_taxable_sales_amt_ten_new += abs(line.debit - line.credit)                
            value_taxable_sales_amt_five=tax_five_percentage_amt
            value_taxable_sales_amt_ten=tax_ten_percetnage_amt
            print('value_taxable_sales_amt_ten',value_taxable_sales_amt_ten)

            #value_taxable_sales_amt_five
            payable_list.append([0, 0, {'label': 'Taxable Good at 5% Rate', 'value_taxable_sales_amt': value_taxable_sales_amt_five_new,
                                        'tax_rate': '5%', 'tax_payable_amt': round((value_taxable_sales_amt_five_new*5)/100,2) }])
            tax_payable_amt1=round((value_taxable_sales_amt_ten * 0.10),3)
            _logger.info('tax_payable_amt1=%s'%tax_payable_amt1)

            #value_taxable_sales_amt_ten
            payable_list.append(
                [0, 0, {'label': 'Taxable Good at 10% Rate', 'value_taxable_sales_amt': value_taxable_sales_amt_ten_new,
                        'tax_rate': '10%', 'tax_payable_amt': round((value_taxable_sales_amt_ten_new*10)/100,2)}])
            ...
            #calculate cn amount
            cn_account_line_obj = self.env['account.move.line'].search(
                [('move_id.invoice_date', '>=', self.from_date), ('move_id.invoice_date', '<=', self.to_date),
                 ('move_id.state', '=', 'posted'), ('move_id.move_type', 'in', ['out_refund'])])
            cn_result=0
            for cn_line in cn_account_line_obj:
                 if cn_line.name =='S-10':
                    cn_result += (cn_line.debit-cn_line.credit)
                # if cn_line.tax_ids:
                    # for tax in cn_line.tax_ids:
                        # if tax.tax_code_id.name not in ['S-0']:
                        #     cn_result+=(cn_line.price_total - cn_line.price_subtotal)
                        

                        # if tax.tax_code_id.name not in ['S-0'] and tax:
                        #     cn_result+=(cn_line.price_total - cn_line.price_subtotal)    
    ...
    def calculate_part_d(self):
        ...
        for move_line in account_move_line_obj:
            ...
            if move_line.tax_ids.tax_code_id.name=='ESA':
                field_18_b1_total=field_18_b1_total+(move_line.credit)
            if move_line.tax_ids.tax_code_id.name == 'ESB':
                field_18_b2_total = field_18_b2_total + (move_line.credit)
            if move_line.tax_ids.tax_code_id.name=='ESC-A':
                field_18_b31_total=field_18_b31_total+(move_line.credit)
            if move_line.tax_ids.tax_code_id.name == 'ESC-B':
                field_18_b32_total = field_18_b32_total + (move_line.credit)
            if move_line.tax_ids.tax_code_id.name == 'ESC-C':
                field_18_b33_total = field_18_b33_total + (move_line.credit)
            if move_line.tax_ids.tax_code_id.name == 'ESV-6':
                field_18_c_total = field_18_c_total + (move_line.credit)
    ...
    def calculate_part_e(self):
        ...
        field_21_total=0
        for move_line in account_move_line_obj:
            print(move_line.tax_ids.name,move_line.move_id.name,move_line.credit,move_line.debit)
            if move_line.tax_ids.tax_code_id.name=='EPC-A':
                field_19_total=field_19_total+(move_line.debit)
            if move_line.tax_ids.tax_code_id.name=='EPC-B':
                field_20_total=field_20_total+(move_line.debit)
            if move_line.tax_ids.tax_code_id.name=='EPC-C':
                field_21_total=field_21_total+(move_line.debit)
...
class sstaccountmove(models.Model):
    ...
    def button_draft(self):
        print('buton_draft')
        res=super(sstaccountmove, self).button_draft()
        for data in self:
            sst_ret_obj=self.env['sst.cal'].search([('state','=','process')])
            for record in sst_ret_obj:
                print(record)
                if record.from_date and record.to_date and data.invoice_date:
                    if record.from_date<=data.invoice_date<=record.to_date:
                        txt='{} already processed ,posting is not allowed'
                        raise ValidationError(txt.format(record.name))
                    else:pass
        return res      
