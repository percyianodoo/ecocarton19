...
class SecurityController(http.Controller):
    ...
    def access_rights(self, model, operation='read', **kw):
        try:
            return request.make_json_response(request.env[model].check_access(operation) is None) # TODO REMOVE
    ...
    def access_rules(self, model, ids, operation='read', **kw):
        ...
        try:
            return request.make_json_response(
                request.env[model].browse(ids).check_access(operation) is None # TODO REMOVE
            )
        except (AccessError, UserError):
            pass
        return request.make_json_response(False)
    ...
    def access(self, model, ids, operation='read', **kw):
        ids = tools.common.parse_ids(ids)
        try:
            return request.make_json_response(request.env[model].browse(ids).check_access(operation) is None)
        except (AccessError, UserError):
            pass
        return request.make_json_response(False)
